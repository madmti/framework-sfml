...
void proccess(string& line) {
    ...
    if (tp == "map" || name == "0") return;
    vector<string> parent_path = getFullParentPath(line);

    cout << "\t// " << line << endl;
    if (tp == "list") cout << "\tassing_list(";
    else cout << "\tassing(";
    print_path_dot(name, parent_path);
    cout << ", yaml";
    if (!(tp == "std::string" || tp == "int" || tp == "bool" || tp == "list")) {
        name = parent_path.back();
...
int main(int argc, char** argv) {
    ...
    cout << "void load_t_" << name << "(t_" << name << "& " << name << ") {" << endl;
